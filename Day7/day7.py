...
class TreeNode:
    ...
    def check_for_size(self, maxSize):

        totalDirSize = 0

        # check each directory to see if its less than or equal to the maxSize
        for i in range(len(self.directories)):
            totalDirSize += self.directories[i].check_for_size(maxSize)

            if int(self.dirSize[i]) <= maxSize:
                print(f"{self.directories[i].name} {self.dirSize[i]}")
                totalDirSize += (int(self.dirSize[i]))
            else:
                print(
                    f"{self.directories[i].name} {self.dirSize[i]} > {maxSize}")
