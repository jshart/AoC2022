...
def viewScore(x, y, raw):
    ...
    for i in range(y-1,-1,-1):
        print("^{}{}".format(raw[x][i].h,raw[x][y].h),end='')
        nc+=1

        if (raw[x][i].h>=raw[x][y].h):
            break
    
    # count trees south
    for i in range(y+1,maxy):
        print("v{}[{}]{}".format(i,raw[x][i].h,raw[x][y].h),end='')
        sc+=1
    ...
    # nc = 1 if nc==0 else nc
    # sc = 1 if sc==0 else sc
    # ec = 1 if ec==0 else ec
    # wc = 1 if wc==0 else wc

    total=nc*sc*ec*wc

    print("up:{} left:{} down:{} right:{} total:{}".format(nc,wc,sc,ec,total))
