...
def partialOverLap(s1, e1, s2, e2):
    if (s1 >= s2 and s1 <= e2) or (e2 >= s1 and e2 <= e1):
        print("R1 in R2 {},{}-{},{} OL:{}".format(s1, e1, s2, e2, 1))
        return 1

    if (s2 >= s1 and s2 <= e1) or (e1 >= s2 and e1 <= e2):
        print("R1 in R2 {},{}-{},{} OL:{}".format(s1, e1, s2, e2, 1))
        return 1
...
for line in Lines:
    ...
    result = partialOverLap(int(sl[0]), int(sl[1]), int(sl[2]), int(sl[3]))
